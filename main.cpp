...
void testDisplay() {
	...
	Rectangular Rectangular(5, 4, 3);
	Rectangular.setPosition(20, 0, 20);
	Rectangular.setRotation(0);
	Rectangular.setColor(1, 0, 0);
	Rectangular.draw();
	
	Triangle Triangle(5, 4, 30, 6);
	Triangle.setPosition(-20, 0, 20);
	Triangle.setRotation(45);
	Triangle.setColor(0, 1, 0);
	Triangle.draw();

	TrapezoidalPrism TrapezoidalPrism(5, 3, 1, 2, 4);
	TrapezoidalPrism.setPosition(-20, 0, -20);
	TrapezoidalPrism.setRotation(0);
	TrapezoidalPrism.setColor(0, 0, 1);
	TrapezoidalPrism.draw();
	
	Cylinder Cylinder(2, 6,1,1);
	Cylinder.setPosition(20, 0, -20);	// For setting position of cylinder
	Cylinder.setRotation(90);			// For setting rotation of cylinder
				...
				switch(msg.type) {
					...
					case 'M':
						{
							...
							for(unsigned int k = 0; k < models.size(); k++) {
								...
								for (vector<ShapeInit>::iterator it = vm.shapes.begin(); it != vm.shapes.end(); it++) {
									...
									float x_value = it->xyz[0];//store the x value
									float y_value = it->xyz[1];//store the y value 
									float z_value = it->xyz[2];//store the z value 
									float r = it->rgb[0];
									float g = it->rgb[1];
									float b = it->rgb[2];//got the color of the shape

									float rotation_angle = it->rotation;
									...
									{
										float x_length = it->params.rect.xlen;//got the length from server
										float y_length = it->params.rect.ylen;//got the height from server
										float z_length = it->params.rect.zlen;//got the width from server

										Shape* shape1 = NULL;
										shape1 = new Rectangular(x_length, z_length, y_length);
										shape1->setPosition(x_value, y_value, z_value);
										shape1->setColor(r, g, b);//set the color of the shape
										shape1->setRotation(rotation_angle);
										otherVehicles[vm.remoteID]->addShape(shape1);
										otherVehicles[vm.remoteID]->draw();//using the draw routine to draw the shape
									}break;

									case TRIANGULAR_PRISM:
									{
										float a_len = it->params.tri.alen;
										float b_len = it->params.tri.blen;
										float angle_value = it->params.tri.angle;
										float depth_value = it->params.tri.depth;//got the parameter of triangle prism from server

										Shape* shape2 = NULL;
										shape2 = new Triangle(a_len, b_len, angle_value, depth_value);
										shape2->setPosition(x_value, y_value, z_value);
										shape2->setColor(r, g, b);
										shape2->setRotation(rotation_angle);
										otherVehicles[vm.remoteID]->addShape(shape2);
									...
									{
										float trap_alength = it->params.trap.alen;
										float trap_blength = it->params.trap.blen;
										float trap_height = it->params.trap.height;
										float trap_aoff = it->params.trap.aoff;
										float trap_depth = it->params.trap.depth;
										Shape* shape3 = NULL;
										shape3 = new TrapezoidalPrism(trap_alength, trap_blength, trap_height, trap_aoff, trap_depth);
										shape3->setPosition(x_value, y_value, z_value);
										shape3->setColor(r, g, b);
										shape3->setRotation(rotation_angle);
										otherVehicles[vm.remoteID]->addShape(shape3);
										otherVehicles[vm.remoteID]->draw();
									}break;
									...
									case CYLINDER:
									{
										float cylinder_radius = it->params.cyl.radius;
										float cylinder_depth = it->params.cyl.depth;
										bool cylinder_rolling = it->params.cyl.isRolling;
										bool cylinder_steering = it->params.cyl.isSteering;

										Shape* shape4 = NULL;
										shape4 = new Cylinder(cylinder_radius, cylinder_depth, cylinder_rolling, cylinder_steering);
										shape4->setPosition(x_value, y_value, z_value);
										shape4->setColor(r, g, b);
										shape4->setRotation(rotation_angle);
										otherVehicles[vm.remoteID]->addShape(shape4);
										otherVehicles[vm.remoteID]->draw();
									}break;
					...
					case 'S': 
						...
						for (unsigned int i = 0; i < states.size(); i++) {
							...
							//find and store the vehicle with ID=1
